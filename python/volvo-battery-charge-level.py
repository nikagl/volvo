...
try:
    url = "https://api.volvocars.com/connected-vehicle/v1/vehicles/" + config["volvo_vin"] + "/battery-charge-level"
    print("URL: {}".format(url))
    vehicledata = requests.get(
        url,
        headers= {
    ...
    print("Result: {}".format(vehicledata))
    vehicledatajson = json.dumps(vehicledata.json(), indent=4)
    print("\nTimestamp: {}".format(json.loads(vehicledatajson)["data"]["batteryChargeLevel"]["timestamp"]))
    print("Value: {}".format(json.loads(vehicledatajson)["data"]["batteryChargeLevel"]["value"]))
    print("Unit: {}".format(json.loads(vehicledatajson)["data"]["batteryChargeLevel"]["unit"]))
except requests.exceptions.RequestException as error:
